          ...
          T: Send + Sync + Copy + 'static + Add<T> + AddAssign<T> + Clone + Default + Display + Div<T> + DivAssign<T> + FromStr + LowerExp + Mul<T> + MulAssign<T> + PartialEq<T> + PartialOrd<T> + Rem<T> + RemAssign<T> + Sub<T> + SubAssign<T> + UpperExp
{
    ...
    delegate! {
        to self.namespace {
            ...
            pub async fn insert(&self, key: K, value: Vec<T>) -> Result<()>;

            pub async fn update(&self, key: &K, value: Vec<T>) -> Result<()>;

            pub async fn replace(&self, key: &K, value: Vec<T>) -> Result<()>;
            ...
            pub async fn retrieve(&self, key: &K) -> Result<Vec<T>>;
            ...
            pub async fn upsert(&self, key: K, value: Vec<T>) -> Result<()>;
